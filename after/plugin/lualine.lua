...
-- gets the colors from a highlight group
...
ins_left {
    function()
        ...
        -- just use the first line lole
        return split(diagnostics[max_severity_idx].message, "\n")[1]
    end,
    color = function()
        ...
        local fg = get_hl("Diagnostic" .. severities[max_severity]).foreground
        return { fg = fg, gui = "bold,italic" }
    end,
    icons_enabled = true,
    icon = "󰋗",
}
